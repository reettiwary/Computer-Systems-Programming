...
//Intro Paragraph:
//This program uses a binary tree structure which helps represent //the cuts made to a floor plan as internal nodes and the //components of the floor produced from the splits as leaf nodes. //We used pointers and dereferencing to manipulate the //characteristics of individual nodes passed to the function, and //we used recursion to traverse the left and right sides of the //tree whenever a function required it. We checked for null //pointers in all of our functions. 

//Members: shreyav3, tkarim3, rtiwary2


// Function: is_leaf_node 
// Return 1 if the given slicing tree node is a leaf node, and 0 otherwise.
int is_leaf_node(node_t* ptr) {
//here we set a condition where we first determine whether the //pointer passed to the function points to a valid tree node, and //if it is, determines if it is a leaf node
 if (!ptr) //if ptr passed to function is NULL
 {
   return 0; //not a valid node, return 0
 }
 if (!(ptr->left) && !(ptr->right)) //if node has no left or right children, then it is a leaf node
 {
   return 1; //leaf node, return 1
 }
 // TODO: (remember to modify the return value appropriately)
 return 0; //not a leaf node, return 0
}
...
//here we set a condition where we first determine whether the //pointer passed to the function points to a valid tree node, and //if it is, determines if it is an internal node
 if (!ptr) //if ptr passed to function is NULL
 {
   return 0; //not a node, so return 0
 }
 if ((ptr->left) || (ptr->right)) //if ptr has a child node
 {
   return 1; //internal node, return 1
 }
 return 0; //not an internal node, return 0
}
...
{
 // TODO: (remember to modify the return value appropriately)
//here we set a conditional statement to check if the pointers //passed to the function are valid (not NULL) 
 if (!a || !b)
 {
   return 0; //NULL pointers, so we return a 0
 }
//then we have another conditional statement where if the node //pointed to by a equals the node pointed to by b
 if (a == b)
 {
   return 1; //b and a point to the same node, we return 1
 }

//we make a recursive call to the function for the left side of a’s node, and if we find b’s node there, we return 1 to indicate the node was found
 if (is_in_subtree(a->left, b))
 {
   return 1; //return a 1 to indicate node b was found 
 }
 return is_in_subtree(a->right, b); //check the right side of a’s node if b’s node wasn’t found in the left side, the outcome of this will be returned if none of the previous conditions were satisfied
}
...
{
 // TODO:
 int temp; //initialize temp variable to use as intermediate variable in swapping ptr module’s height and width
 temp = ptr->module->h;//temp stores the value of the height of ptr’s module
 ptr->module->h = ptr->module->w;//the value of ptr’s module height becomes the value of its width
 ptr->module->w = temp; //the value of ptr module’s width becomes temp, so height and width of the module are finally swapped
}
...
void recut(node_t* ptr) {
 ...
 if (ptr->cutline == H)// if ptr has a horizontal cutline (H)
 {
   ptr->cutline = V; // the ptr cutline is changed to vertical (V)
 }
 else //otherwise, if ptr has a vertical cutline
 {
...
void swap_module(node_t* a, node_t* b) {
 ...
 // TODO:
 module_t *temp; //create a module temp to be an intermediate module when swapping modules of nodes a and b
 temp =  a->module; //temp is set to module of node a
 a->module = b->module; //the module of node a now takes on the characteristics of the module of node b
 b->module = temp; //the module of node b now takes on the characteristics of the module of node a, stored in temp
}
...
void swap_topology(node_t* a, node_t* b) {
 ...
 // TODO:
 // swapping the parents of nodes a and b
 node_t *temp; //create a node temp to be an intermediate module when swapping 
 temp = a->parent; //temp takes on characteristics of the parent of node a
 a->parent = b->parent; //the parent of node a takes on the characteristics of the parent of node b
 b->parent = temp; //the parent of node b takes on the characteristics of the parent of node a, stored in temp


 //swapping the children of the switched parents of nodes a and b
 if(a->parent->left == b) //if the left child of the new parent node of a is b, then set it back to a
 {
   a->parent->left = a; //change it to a
 }
 else //if the right child of the new parent node of a is b, then set it back to a
 {
   a->parent->right = a; //change it to a
 }
 if(b->parent->left == a) //if the left child of the new parent node of b is a, then set it back to b
 {
   b->parent->left = b; //change it to b
 }
 else //if the right child of the new parent node of b is a, then set it back to b
 {
   b->parent->right = b; //change it to b
 }
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
 ...
 // TODO:
 // recursion on left and right sides of the tree
 postfix_traversal(ptr->left, nth, expression); //traverse the left subtree with recursive call to the function
 postfix_traversal(ptr->right, nth, expression); //traverse the right subtree with recursive call to the function
 if (ptr->module) //if module pointer exists
 {
   expression[*nth].cutline = UNDEFINED_CUTLINE; //set cutline of the data in the *nth index of the expression array to be undefined
   expression[*nth].module = ptr->module; //set module of the data in the *nth index of the expression array to be ptr’s module
 }
 if (ptr->cutline != UNDEFINED_CUTLINE) //if ptr has either a horizontal or vertical cutline
 {
   expression[*nth].module = NULL; //set module of the data in the *nth index of the expression array to NULL
   expression[*nth].cutline = ptr->cutline; //set cutline of the data in the *nth index of the expression array to be ptr’s cutline
 }
 (*nth)++; //increment the index *nth
}
...
{
 // TODO:
 if (!ptr) //conditional statement where if ptr is null
 {
...
//create a for loop where as long as i is 0 and less than
//num_modules, execute the command inside and increment by 1
 for(int i = 0; i < num_modules; i++)
 {
//sum is incremented by the value stored at the resource attribute of index i of our modules array
   sum += modules[i].resource; 
 }
 return sum; //then we return the current value of sum 
}
...
node_t* init_slicing_tree(node_t* par, int n) {
 ...
 // TODO:
 node_t *ptr = (node_t*)malloc(sizeof(node_t)); //allocate space for our new node *ptr


 //Base Case
 if (n == num_modules - 1) //if n equals number of input modules minus 1
 {
   ptr->parent = par; //ptr’s parent is equal to par
   ptr->module = &modules[n];//ptr’s module is set to the module at index n of the modules array
   ptr->cutline = UNDEFINED_CUTLINE; //ptr’s cutline is equal to UNDEFINED_CULTURE
   return ptr; //return ptr
 }
 //Internal Node
 ptr->parent = par; //ptr’s parent is equal to par
 ptr->module = NULL; //ptr’s module is set to NULL
 ptr->cutline = V; //ptr’s cutline is equal to vertical (V)


 //Right Child
 ptr->right = (node_t*)malloc(sizeof(node_t)); //allocate space for the right child of our node ptr 
 ptr->right->parent = ptr; //parent of ptr’s right child is set to ptr
 ptr->right->module = &modules[n]; //module of ptr’s right child is set to the module at index n of the modules array
 ptr->right->cutline = UNDEFINED_CUTLINE; //cutline of ptr’s right child is equal to UNDEFINED_CUTLINE



 //Left Child (recursive call)
 ptr->left = init_slicing_tree(ptr, n+1); //a recursive call is made on the function, this time for ptr and index n+1, and the result of that is allocated to ptr’s left child 
 return ptr; // return ptr
}
